...
if ($('storeTagline')) $('storeTagline').textContent = settings.tagline;
...
const searchOk = !q || [p.name_en,p.name_hi,p.name_hinglish,p.name,p.category,p.subcategory].join(' ').toLowerCase().includes(q);
...
root.innerHTML = list.map(p=>  <article class="product-card ${viewMode}">   <img class="product-img" src="${p.image}" alt="${getName(p)}">   <div class="product-body">   <div class="badge">${p.badge || 'Featured'}</div>   <h4>${getName(p)}</h4>   <div class="meta">${p.category} • ${p.subcategory || ''}</div>   <div class="price-row"><strong>₹${p.price}</strong><button class="add-btn" data-id="${p.id}">${t('add')}</button></div>   </div>   </article>  ).join('');
...
if ($('successToast')) {
$('successToast').textContent = t('orderSuccess');
show($('successToast'));
setTimeout(()=>hide($('successToast')), 2500);
}
hide($('cartSheet'));
};
}
